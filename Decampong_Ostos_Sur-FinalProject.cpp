...
int countPassing(float g_Sci, float g_Math, float g_Eng, float g_Art, float g_hstry) {
    int passCount = 0;
    if (g_Sci >= 75) passCount++;
    if (g_Math >= 75) passCount++;
    if (g_Eng >= 75) passCount++;
    if (g_Art >= 75) passCount++;
    if (g_hstry >= 75) passCount++;
    return passCount; 
}
...
// This code demonstrates modular programming in which it organizes tasks into reusable functions, it uses a do-while loop for input validation to ensure only valid grades are accepted, and it also applies stream manipulator from <iomanip> to neatly format numeric output with two decimal places
